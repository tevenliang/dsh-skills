...
class TokenManager:
    ...
    def gen_real_msToken(cls) -> str:
        ...
        _proxy = (cls.proxies or {}).get("https://") or (cls.proxies or {}).get("http://")
        with httpx.Client(transport=transport, proxy=_proxy) as client:
            try:
...
class SecUserIdFetcher:
    ...
    async def get_sec_user_id(cls, url: str) -> str:
        ...
        try:
            ...
            _proxy = (TokenManager.proxies or {}).get("https://") or (TokenManager.proxies or {}).get("http://")
            async with httpx.AsyncClient(
                    transport=transport, proxy=_proxy, timeout=10
            ) as client:
...
class AwemeIdFetcher:
    ...
    async def get_aweme_id(cls, url: str) -> str:
        ...
        # 重定向到完整链接
        # 2026-07-21: 不再传 proxy=None — VPN 按域名自动路由, httpx 默认即可
        transport = httpx.AsyncHTTPTransport(retries=5)
        async with httpx.AsyncClient(
