...
class BaseCrawler:
    ...
    def __init__(
        ...
        # 2026-07-21 fix: 空字符串值视为"不用代理"，避免 httpx fallback 到系统代理
        if isinstance(proxies, dict) and any(v for v in proxies.values() if v):
        ...
        # B3 fix (2026-07-16): 移除冗余的 asyncio.Semaphore(max_tasks) 创建.
        # 原代码在 __init__ 同步路径建 semaphore, 绑定到 (尚未存在) 的 event loop,
        # Python 3.13 的 resource_tracker 在 loop 关闭时会报 leaked semaphore.
        # 此 semaphore 在整个 base_crawler 里没有 .acquire() 调用, 完全冗余,
        # 删除即可根除泄漏, 无功能损失.
        ...
        # 2026-08-22: 加 connect 超时，防止 SSL 握手阶段卡死
        self.timeout = httpx.Timeout(timeout, connect=8)
        # 异步客户端 / Asynchronous client
        # 2026-09-02 VM 适配: httpx 0.28 起 proxies= 改名 proxy= (dict → str)
        _proxy = None
        if isinstance(self.proxies, dict):
            _proxy = self.proxies.get("https://") or self.proxies.get("http://")
