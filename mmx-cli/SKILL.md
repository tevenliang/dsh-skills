---
name: mmx-cli
description: Use mmx CLI to generate text, images, video, speech, and music, and
  to understand/analyze images via MiniMax AI platform. PRIMARY TOOL for image
  understanding — use `mmx vision describe --image <path>` when the user shares
  an image, asks what an image shows, or needs visual analysis. Also use when
  the user wants to create media content, chat with MiniMax models, perform web
  search, or manage MiniMax API resources from the terminal.
...
- **图片生成必须用 `mmx image generate`，不要自己 curl minimaxi 的图片 API**。minimaxi 的图片 endpoint 是 `/v1/image_generation`（**单数 image，没有 s**）；OpenAI 风格的 `/v1/images/generations` 在 minimaxi 返回 404。
- **base_url 不要带 `/v1` 后缀**。正确：`https://api.minimaxi.com`。错误：`https://api.minimaxi.com/v1`。mmx CLI 内部会自己拼 `/v1/...`，base_url 多带 `/v1` 会拼成 `/v1/v1/...`，所有 endpoint 都 404。
...
## Image Understanding (Primary Use)

**Use this as the default when users share or ask about images.**

```bash
mmx vision describe --image <local-path> --output json --quiet
```

- Local path: works with absolute paths (e.g. `/var/folders/.../codex-clipboard-xxx.png`)
- Output JSON: `--output json --quiet` gives clean JSON for parsing
- Default prompt: `"Describe the image."` — override with `--prompt "<question>"`
- Model: uses MiniMax-M3 VLM (vision-optimized model)

Example — understand a screenshot:
```bash
mmx vision describe --image /var/folders/wz/xxx.png --output json --quiet
```
...
vm 上 bridge 用 markdown 模式发 reply（不是 create），reply 模式绕过了飞书 p2p 私聊 chat 成员检查。**所以 vm codex 不用主动调 lark-cli，只需要在回复里用 markdown 写图片 URL，飞书就会直接渲染图片**。

**工作流**（在 vm 上 codex 收到"生成 X 图"指令时）：

1. **生成图**：`mmx image generate --prompt "..." --output /tmp/x.jpg --quiet`（用 minimaxi M3 文字模型 key）
2. **复制到 webdav 根目录**（公网可访问）：
   ```bash
   cp /tmp/x.jpg /home/ubuntu/webdav/x.jpg
   ```
3. **回复时输出 markdown**：
   ```
   ![X](http://175.178.210.156:9001/x.jpg)
   ```
4. bridge 自动用 markdown + reply 模式发出去，飞书直接渲染图片

**公网 webdav URL 模板**：`http://175.178.210.156:9001/<文件名>`（webdav 根目录是 `/home/ubuntu/webdav/`，steven_vault 是其中一个子目录，不在 URL 路径里）

**为什么不能直接用 lark-cli 发图**：
- lark-cli `im +messages-send` 调的是 `im.v1.message.create`（不是 reply）
- p2p 私聊里 bot 不在 chat 成员里 → 飞书 API 返回 230002 `Bot can NOT be out of the chat`
- bridge 之所以能 work 是因为它走 reply 接口（指向用户最新消息的 message_id），reply 绕过成员检查
