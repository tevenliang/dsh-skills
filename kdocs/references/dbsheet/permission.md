...
- **前置检查**：文档已开通**高级权限 / 内容权限**能力；否则可能返回业务错误。
...
- **前置检查**：`task_id`须来自「新增/更新/删除自定义角色」等异步接口返回。
...
- **前置检查**：文档已开启高级权限；`cloud_permission_id` 可与 `dbsheet.permission_list_roles` 返回体中的云权限 ID 对照填写。
