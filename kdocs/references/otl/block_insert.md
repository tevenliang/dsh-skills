...
向智能文档插入一个或多个块，适合在指定父块下按位置追加段落、列表、表格、图片等结构化内容。
...
> 返回结果因内容和文档状态不同而异，以 code == 0 判断成功
> 待插入的节点类型和属性可参考 `references/otl/node.md`
> 当 blockId 为 `doc` 时，index 必须 >= 1，因为 doc 的首个子节点必须是 title（全局唯一）；正文开头插入应设 index 为 1
...
- `params` (object, 必填): 插入操作配置
  ...
  - `content` (array, 常用): 待插入的块内容数组。常见块类型：`paragraph`、`heading`、`picture`、`blockQuote`、`codeBlock`、`table`、`hr`、`column`。完整定义见 `references/otl/node.md`
