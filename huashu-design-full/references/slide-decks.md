...
### 5. 键盘翻页在 iframe / 预览面板里失效（←/→ 不响应）

`deck_stage.js` 的翻页监听 `document` 的 keydown。当 deck 在 **iframe 内嵌环境**（如 DSH Web GUI 的文件预览 / 侧边栏浏览器）打开时，键盘事件默认传不到 iframe 内的页面，导致方向键、空格全部失效。用户看到单页卡住，以为是脚本坏了。

**对策**：**永远不要只靠键盘导航**。至少提供一层不依赖键盘的交互：
- 常驻可见的 **◀ ▶ 按钮**（`position:fixed`，半透明，hover 变亮），点击即翻；
- 点击页面区域翻页（左 18% 区域=上一页，其余=下一页）；
- 键盘仍保留，但作为「附带支持」而非唯一入口。
外部验证时若只在 1920×1080 截图、没在真实 iframe 预览里点过按钮，这个坑发现不了——交付前务必在目标预览环境里实际点一次。

### 6. 自定义 scale-to-fit 居中数学错 → 整页被推到右下角裁切（错位）

想自己写适配缩放时，常见错误：容器 `top:50%; left:50%` 先把左上角顶到视口中心，又叠加一个**正向** `translate((vw-W·s)/2, (vh-H·s)/2)`。两者叠加等于把居中偏移算了两次，结果整页被推到右下角、超出可视区被裁切——在非 1920×1080 窗口下尤其明显（1920×1080 时 translate 恰为 0，反而看不出问题，这是为什么只在单尺寸截图验不出）。

**正确做法**（也就是 `deck_stage.js` 自身 `_updateScale` 的做法——它运行时把 `stage.style.top/left` 覆写为 `0`）：
- 容器锚在 `top:0; left:0`，再用 `translate((vw - W·s)/2, (vh - H·s)/2) scale(s)`，`transform-origin: top left`；或
- 反过来：保留 `top:50%; left:50%`，但 translate 用 `translate(-50%, -50%) scale(s)`（百分比相对元素自身尺寸）。
两种二选一，**不要混用 `top:50%` 与正向像素 translate**。
验证：用 Playwright 在 1280×720 / 1440×900 / 1920×1080 三个尺寸各测一次 `#deck` 的 `getBoundingClientRect()`，确认左上角分别在 ((vw-Ws)/2,(vh-Hs)/2)、不溢出、不被裁切，才算过关。
