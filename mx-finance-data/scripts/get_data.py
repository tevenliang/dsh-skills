...
# 授权流程：取数前由 query_mx_finance_data 内联调用 ensure_auth()，
# 避免每次取数都额外跑一轮 auth.py ensure 的工具调用。
from auth import ensure_auth, AUTH_OK, AUTH_NEED_USER, AUTH_ERROR

# 兜底默认值：ensure_auth 正常会返回 api_key_url（来自 create 接口的 apiKeyUrl），
# 此值仅在接口未返回时使用
...
def _load_em_api_key() -> str:
    ...
    两处都为空则返回空串。ensure_auth() 落盘成功后此处即可读到。
    """
...
class _AuthRevoked(Exception):
    """EM_API_KEY 服务端失效（HTTP 401 或业务码 401），需清掉落盘 key 并触发重新授权。"""


def _clear_em_api_key_file() -> None:
    """删除落盘的 EM_API_KEY 文件，下次调用 ensure_auth 时会自动走重新授权流程。"""
    cred_path = Path.home() / ".mx-skills" / "em_api_key"
    try:
...
def _handle_auth_revoked(reason: str, result: Dict[str, Any]) -> Dict[str, Any]:
    """
    EM_API_KEY 失效时的统一处理：清掉落盘 key、重新生成授权链接、返回 need_auth。
    这样 agent 在同一次工具调用里就能拿到新的 authUrl，用户扫码后重发指令即可，
    不必先看到 401 错误、再重发一次才能拿到二维码。
    """
...
def _check_business_status(api_result: Any) -> Optional[str]:
    ...
    # 401 业务码 = api_key 服务端失效，单独抛 _AuthRevoked 走重新授权流程
    if code in (401, "401") or status in (401, "401"):
        raise _AuthRevoked(f"业务码 401: code={code}, status={status}")
...
async def query_mx_finance_data(
    ...
    # 取数前内联授权检查：已有 key 零开销；未授权则返回结构化 need_auth，
    # 由调用方把 authUrl 展示给用户，完成后重发指令，下次自动落盘并取数。
    auth = ensure_auth()
    if auth["status"] == AUTH_NEED_USER:
        ...
        result["auth_message"] = (
            "尚未完成授权。请扫码授权（authUrl），或到上述 apiKeyUrl 手动复制 apikey。"
            "完成后重新发送原指令，下次调用会自动检测并落盘 EM_API_KEY，随后取数。"
        )
        return result
    ...
    if not api_key:
        # ensure_auth 返回 ok 但读不到 key 的兜底（理论上不应发生）
    ...
    try:
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            resp.raise_for_status()
            data = resp.json()
        # 业务码 401 时 _check_business_status 会抛 _AuthRevoked，放在 try 内被捕获
        status_err = _check_business_status(data)
    except httpx.HTTPStatusError as exc:
        if exc.response.status_code == 401:
            return _handle_auth_revoked(f"HTTP 401: {exc.response.text[:100]}", result)
...
def run_cli() -> None:
    ...
    async def _main() -> None:
        ...
        if result.get("need_auth"):
            # 未授权：把授权入口打到 stdout 供调用方解析，退出码 10 与 auth.py EXIT_NEED_USER 一致
            print("need_auth: true")
            if result.get("authUrl"):
                print(f"authUrl: {result['authUrl']}")
                print(f"apiKeyUrl: {result.get('apiKeyUrl', AUTH_API_KEY_URL_FALLBACK)}")
            if result.get("auth_message"):
                print(result["auth_message"])
            print("完成后重新发送原指令即可，下次调用会自动落盘 EM_API_KEY 并取数。")
            sys.exit(10)
