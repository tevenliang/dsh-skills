...
description_zh: 批量资料整理工具，支持最多 30
  项素材（URL/PDF/Word/图片/纯文本），自动提取要点、去重归类，输出带目录和来源溯源的结构化研究笔记，含关键词索引，支持保存为文件。
description_en: Batch material organizer that ingests up to 30 items (URLs,
  PDFs, Word docs, images, plain text), extracts key points, deduplicates, and
  outputs a structured research-note Markdown document with table of contents,
  source attribution, keyword index, and file export.
author: TPD
