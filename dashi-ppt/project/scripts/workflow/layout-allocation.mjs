...
export function selectDiverseLayoutCandidates(
  ...
  const basePool = uniqueCandidates(candidates);
  const pool = basePool.slice(0, candidateLimit);
  if (pool.length === 0) {
    throw new Error(`layout allocation requires ${requestedCount} candidates, received 0`);
  }
  // 放宽:版式不足时循环复用已候选版式补齐,避免主题版式库偏少时无法生成扩展 slides。
  while (pool.length < requestedCount) {
    pool.push(basePool[pool.length % basePool.length]);
  }
