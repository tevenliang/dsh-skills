...
description: "Unified Lark/Feishu CLI suite covering messaging, documents,
  collaboration, scheduling, and more. Provides 200+ commands across 11 business
  domains including instant messaging (IM), cloud documents, spreadsheets,
  base/tables, calendars, mail, tasks, wikis, video conferences, and custom
  integrations. Use when working with Lark/Feishu through CLI or needing to
  understand Lark API capabilities for: sending/receiving messages, managing
  documents and files, creating spreadsheets, managing tasks and calendars,
  searching conversations, building custom workflows, or accessing any Lark
  business application."
description_zh: 飞书/Lark 全能套件（消息、文档、表格、日历、任务、Wiki 等 11 个业务域）
description_en: "Lark/Feishu unified CLI: messaging, docs, sheets, calendar,
  tasks, wiki & more"
version: 1.0.3
...
#### Wiki / 知识库专属 IDs（容易混，2026-07-07 实测确认）

- **`node_token`（节点树 ID）**：wiki 节点在空间树里的唯一标识（`wikcnXXX` 前缀）。`wiki +node-list` 响应 `nodes[].node_token` 即此。**用于**：`wiki +node-create --parent-node-token`、`wiki +node-delete --obj-type wiki`、`wiki +node-move`。
- **`obj_token`（文档 ID）**：节点关联的实际文档/对象 ID（`docxXXX` 前缀）。`wiki +node-list` 响应 `nodes[].obj_token`、`wiki +node-create` 响应 `data.obj_token` 即此。**用于**：`docs +update --doc`、`docs +fetch`、`drive +file` 系列。
- **关键区分**：`node-create` 响应**同时**返回 `data.node_token`（节点树 ID）**和** `data.obj_token`（文档 ID），两个都是真字段；任何脚本必须**分别捕获**，不要把其中一个当成另一个。
- **`--obj-type` 必填**（在 `--node-token` 给的是裸 token 时）：`wiki|doc|docx|sheet|bitable|mindnote|slides|file`。删 wiki 节点必须用 `--obj-type wiki --node-token <node_token>`，**用 obj_token 当 node_token 删会 131005 not found 或 131006 permission denied**。
...
##### Wiki `node-list` 分页真实行为（2026-07-07 实测）

- **单次硬上限 50 条**，响应 `data.has_more=true` + `data.page_token="<base64 游标>"` 时表示还有下一页，下一次调必须带 `--page-token <值>`。
- 响应 `data.nodes`（**不是 `items`！**）是数组，每条 item 同时含 `node_token` + `obj_token` + `title` + `parent_node_token`。
- 完整分页循环示例：
  ```python
  page_token = None
  all_docs = []
  while True:
      cmd = [LARK, "wiki", "+node-list", "--space-id", SPACE_ID,
             "--parent-node-token", parent_token, "--as", "user"]
      if page_token: cmd += ["--page-token", page_token]
      out = run(cmd)
      obj = parse_json(out)
      for it in obj.get("data", {}).get("nodes", []):   # 注意字段名
          all_docs.append(it)
      if not obj.get("data", {}).get("has_more"): break
      page_token = obj.get("data", {}).get("page_token")
      if not page_token: break
      time.sleep(0.2)
  ```
- ⚠️ **bash 变量拼接做分页极易出 bug**（`$page=""` 第一次循环空格被吃掉）。生产代码务必用 Python，且先实测 `len(all_docs) == 本地文件数` 确认全量拿到。
...
### ⚠️ Critical Gotchas — Real-World Traps (2026-07-07 distilled)

这些坑在生产脚本里反复踩到,**任何用 lark-cli 写批量/迁移脚本前必读**:

1. **响应字段名 ≠ 习惯命名**。Lark 多个 list 接口（如 `wiki +node-list`）的数组字段叫 `nodes`（不是 `items`）、`messages`（不是 `msgs`）等。**写代码前先 `print(json.dumps(obj, indent=2))` 看真实 schema**，不要凭印象假设字段名。曾因此把 AI 节点下文档从 50 错建到 280+ 空壳。

2. **`docs +update --content @<file>` 只接受 cwd 相对路径**。绝对路径报 `--file must be a relative path within the cwd`。**用法**：先 `subprocess.run(cmd, cwd=md_dir)`，再传 `"@" + basename`。`docs +media-insert --file` 同理。

3. **`node-delete` 必须显式给 `--obj-type`，且 token 类型必须严格匹配**。
   - 删 wiki 节点 → `--obj-type wiki --node-token <node_token>`（用 obj_token 会 131005）
   - 删文档 → `--obj-type docx --node-token <obj_token>`（或用 `drive +file-delete`）
   - 测试时务必先拿一个无关节点试通，再批量

4. **wiki 允许同名文档共存**（不像文件系统报错）。脚本默认行为下，重跑同一迁移脚本会**静默建出重复文档**。**铁律**：迁移/补缺脚本必须**先 list 父节点下全量文档**（分页，见上）→ 按 title 建索引 → 决定 create / overwrite / 留 1 删余。

5. **lark-cli 不内置限流/重试**。源码实测：任何 HTTP 429 直接报错退出，无退避。应用层必须：① `subprocess.run` 设 `timeout`；② 遇 429 按响应头 `x-ogw-ratelimit-reset` 退避（推荐序列 `[5,15,30,30,60]s` 最多 5 次）；③ 全局令牌桶是根治但工程量大。

6. **写脚本必须可幂等**。一次跑失败/中断，重跑要能从断点续，且不会建出重复/垃圾。**预期**：「再跑一次也安全，只是补缺的」。

7. **Python 后台长跑必用 `python3 -u`（或 `PYTHONUNBUFFERED=1`）**。默认 stdout 是 block buffering，pipe 到 `tee` 时尤其严重——30 分钟跑完才发现日志断在某个时刻，实际程序早就崩了/超时退出了，但日志看不到任何错误。

8. **`parse_json` 必须从首个 `{` 截到最后一个 `}`**。lark-cli 响应**前后都可能带进度行**（`Creating wiki node...` / `Found N node(s)`），有时末尾进度行在 JSON 的 `}` 之后。只截到第一个 `}` 会丢 JSON 末尾一部分；截到末尾（含进度行）会 JSON 解析失败。

9. **`str_replace` 删除成功返回 `result:null`**。判定成功**只看 `ok:true`**，不要 `result=="success"`（否则会全误判失败）。`overwrite`/`append` 才返回 `result:"success"` 或 `"partial_success"`。

10. **成功判定必认 `partial_success`**。含 Obsidian `![[media/...]]` wikilink 或复杂块（表格/代码块）的文档，`docs +update` 会返回 `partial_success`——内容已完整落库，**不是失败**。只匹配 `success` 会把大量成功笔记误报失败。
