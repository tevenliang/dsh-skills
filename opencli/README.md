...
**Step 4 — This skill is already installed**

This skill is part of your Claude Code setup at `~/.claude/skills/opencli/`. It will be auto-loaded on the next session.

> **Current status check:** run `opencli --version` in your terminal. If it prints a version number, you're all set. If `command not found`, run `npm install -g @jackwener/opencli` (Node.js 16+ required).
...
opencli twitter search "claude AI"

# YouTube
opencli youtube search "LLM tutorial"
...
本 Skill 已经放在 `~/.claude/skills/opencli/`,下次新开会话自动加载。

> **当前状态检查:** 终端跑 `opencli --version`。如果输出版本号,一切就绪。如果 `command not found`,跑 `npm install -g @jackwener/opencli`(需要 Node.js 16+)。
