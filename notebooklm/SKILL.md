...
description: Complete API for Google NotebookLM - full programmatic access
  including features not in the web UI. Create notebooks, add sources, generate
  all artifact types, download in multiple formats. Activates on explicit
  /notebooklm or intent like "create a podcast about X"
...
## ⚠️ IMPORTANT: Use WorkBuddy's pre-built environment

> **Do NOT use system Python.** System Python is 3.9.6 which is incompatible (notebooklm-py requires 3.10+).
> 
> **Use the pre-built WorkBuddy venv** which has Python 3.13.12 + notebooklm-py 0.7.0 already installed:
> 
> ```bash
> ~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm <command>
> ```
> 
> **Always prefix commands** with this path. Do NOT run `pip install notebooklm-py` — it is already there.
> 
> If you need a shell alias for convenience:
> ```bash
> alias notebooklm='~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm'
> ```
> (But always use the full path in scripts/code — alias expansion is unreliable in subshells.)

## Installation

**Already installed** in the WorkBuddy venv. No installation needed unless you need to upgrade:

```bash
~/.workbuddy/binaries/python/envs/notebooklm/bin/pip install --upgrade "notebooklm-py[browser]"
```
...
~/.workbuddy/binaries/python/envs/notebooklm/bin/pip install "notebooklm-py[browser] @ git+https://github.com/teng-lin/notebooklm-py@${LATEST_TAG}"
```

⚠️ **DO NOT install from main branch** (`pip install git+https://github.com/teng-lin/notebooklm-py`). The main branch may contain unreleased/unstable changes. Always use PyPI or a specific release tag.

## Prerequisites

**IMPORTANT:** Before using any command, MUST authenticate:

```bash
~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm login          # Opens browser for Google OAuth
~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm list           # Verify authentication works
```
...
> **IMPORTANT:** All commands must use the WorkBuddy venv binary path:
> ```bash
> alias notebooklm='~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm'
> ```
> In scripts and tool calls, always use the full path:
> ```bash
> ~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm <command>
> ```
...
| Task | Command |
|------|---------|
| Authenticate | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm login` |
| Authenticate from browser cookies | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm login --browser-cookies <browser>` |
| Authenticate from one Chromium profile | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm login --browser-cookies 'chrome::Profile 1'` |
| Authenticate from one Firefox container | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm login --browser-cookies 'firefox::Work'` |
| Import every signed-in account into its own profile | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm login --browser-cookies <browser> --all-accounts` |
| Inspect signed-in accounts (read-only, by email) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm auth inspect --browser <browser>` |
| Inspect one browser profile/container | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm auth inspect --browser 'chrome::Profile 1'` |
| Diagnose auth issues | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm auth check` |
| Diagnose auth (full) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm auth check --test` |
| Refresh active profile in place (server-side) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm auth refresh` |
| Refresh active profile from a re-signed-in browser | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm auth refresh --browser-cookies <browser>` |
| Refresh from one Chromium profile | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm auth refresh --browser-cookies 'chrome::Profile 1'` |
| One-shot cookie keepalive (for cron) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm auth refresh --quiet` |
| List notebooks | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm list` |
| Create notebook | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm create "Title"` |
| Set context | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm use <notebook_id>` |
| Show context | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm status` |
| Add URL source | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm source add "https://..."` |
| Add file | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm source add ./file.pdf` |
| Add YouTube | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm source add "https://youtube.com/..."` |
| List sources | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm source list` |
| Delete source by ID | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm source delete <source_id>` |
| Delete source by exact title | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm source delete-by-title "Exact Title"` |
| Wait for source processing | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm source wait <source_id>` |
| Web research (fast) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm source add-research "query"` |
| Web research (deep) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm source add-research "query" --mode deep --no-wait` |
| Web research (query from file) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm source add-research --prompt-file research_query.txt --mode deep` |
| Check research status | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm research status` |
| Wait for research | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm research wait --import-all` |
| Chat | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm ask "question"` |
| Chat (long prompt from file) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm ask --prompt-file question.txt` |
| Chat (specific sources) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm ask "question" -s src_id1 -s src_id2` |
| Chat (with references) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm ask "question" --json` |
| Chat (save answer as note) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm ask "question" --save-as-note` |
| Chat (save with title) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm ask "question" --save-as-note --note-title "Title"` |
| Show conversation history | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm history` |
| Save all history as note | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm history --save` |
| Continue specific conversation | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm ask "question" -c <conversation_id>` |
| Save history with title | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm history --save --note-title "My Research"` |
| Get source fulltext | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm source fulltext <source_id>` |
| Get source guide | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm source guide <source_id>` |
| Generate podcast | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm generate audio "instructions"` |
| Generate (long prompt from file) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm generate audio --prompt-file instructions.txt` |
| Generate podcast (JSON) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm generate audio --json` |
| Generate podcast (specific sources) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm generate audio -s src_id1 -s src_id2` |
| Generate video | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm generate video "instructions"` |
| Generate report | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm generate report --format briefing-doc` |
| Generate report (append instructions) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm generate report --format study-guide --append "Target audience: beginners"` |
| Generate quiz | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm generate quiz` |
| Revise a slide | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm generate revise-slide "prompt" --artifact <id> --slide 0` |
| Check artifact status | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm artifact list` |
| Wait for completion | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm artifact wait <artifact_id>` |
| Download audio | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm download audio ./output.mp3` |
| Download video | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm download video ./output.mp4` |
| Download cinematic video | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm download cinematic-video ./cinematic.mp4` (alias for `download video`) |
| Download infographic | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm download infographic ./infographic.png` |
| Download slide deck (PDF) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm download slide-deck ./slides.pdf` |
| Download slide deck (PPTX) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm download slide-deck ./slides.pptx --format pptx` |
| Download report | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm download report ./report.md` |
| Download mind map | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm download mind-map ./map.json` |
| Download data table | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm download data-table ./data.csv` |
| Download quiz | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm download quiz quiz.json` |
| Download quiz (markdown) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm download quiz --format markdown quiz.md` |
| Download flashcards | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm download flashcards cards.json` |
| Download flashcards (markdown) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm download flashcards --format markdown cards.md` |
| Delete notebook | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm delete -n <id>` (add `--yes` to skip the prompt non-interactively) |
| List languages | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm language list` |
| Get language | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm language get` |
| Set language | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm language set zh_Hans` |
| List profiles | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm profile list` |
| Create profile | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm profile create work` |
| Switch profile | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm profile switch work` |
| Delete profile | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm profile delete old --yes` (`-y`; `--confirm` is a deprecated alias) |
| Rename profile | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm profile rename old new` |
| Use profile (one-off) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm -p work list` |
| Health check | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm doctor` |
| Health check (auto-fix) | `~/.workbuddy/binaries/python/envs/notebooklm/bin/notebooklm doctor --fix` |
...
**Benefits:** Non-blocking, user can do other research, automatic download on completion
