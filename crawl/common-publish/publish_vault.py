...
import json, os, re, shutil, hashlib, time, subprocess
...
MEDIA = VAULT / "media"
VM_HOST = "ubuntu@175.178.210.156"
...
def _load_state() -> dict:
    """从VM读取seen cache（SSH），失败返回空dict"""
    cmd = ["ssh", VM_HOST, "cat", "/home/ubuntu/.agents/skills/crawl/state/.subscription-crawl-cache.json"]
    try:
...
def _save_state(state: dict):
    ...
    try:
        ...
        with open(tmp_path, "r") as f:
            r = subprocess.run(["ssh", VM_HOST, "dd", "of=/home/ubuntu/.agents/skills/crawl/state/.subscription-crawl-cache.json"],
                            stdin=f, capture_output=True, text=True, timeout=10)
        import os; os.unlink(tmp_path)
        if r.returncode != 0:
            print(f"[publish_vault] ⚠️ VM cache save failed: {r.stderr[:100]}")
    except Exception as e:
        print(f"[publish_vault] ⚠️ VM cache save error: {{e}}")
