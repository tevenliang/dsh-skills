...
sys.path.insert(0, str(SKILL_DIR / "common-asr"))
...
OP_PATTERNS = [
    (re.compile(r"🌐 \[(bilibili|douyin|xiaohongshu)\]"), "fetch"),
    (re.compile(r"\[groq\]\s+(?:成功|失败|error)"), "transcribe_groq"),
    (re.compile(r"\[bailian\]\s+(?:成功|失败)"), "transcribe_bailian"),
    (re.compile(r"\[mlx\]\s+\S+.*推理(\d+\.\d+)s"), "transcribe_mlx"),
    (re.compile(r"\[mlx-async\]\s+\S+.*(\d+\.\d+)s"), "transcribe_mlx"),
    (re.compile(r"📝 总结完成"), "summarize_glm"),
    (re.compile(r"⚠️ 总结跳过"), "summarize_glm"),
    (re.compile(r"✅ vault:"), "publish_vault"),
    (re.compile(r"⏭️ .*已缓存"), "skip_dedup"),
    (re.compile(r"\[opencli"), "opencli_op"),
]
...
def preflight_check(skill: str, py: str) -> bool:
    """爬取前健康检查（2026-07-27 新增）.
    
    统一在 supervisor 层做 4 项检查:
    1. Bailian 配额 (check_bailian_quota.py)
    2. Groq key 可用性 (check_groq_balance)
    3. opencli Chrome 连通 (Profile 1/dy2s6y2k)
    4. 凭证目录存在 (~/.agents/credentials/)
    ...
    # 1. Bailian 配额
    print("\n[1/4] Bailian 模型配额")
    quota_py = skill_path / "common_supervisor" / "check_bailian_quota.py"
    try:
        # 2026-07-30: bailian 配额检查加 --no-require-console, console auth 8h 过期但 ASR 仍可用
        # 主路走 Groq (Groq only, 无 fallback), bailian 配额检查仅作参考, quota 查不到时 warning 不 fatal
        r = subprocess.run([py, str(quota_py), "--no-require-console"],
                           capture_output=True, text=True, timeout=60)
        for line in r.stdout.strip().splitlines():
            print(f"  {line}")
        if r.returncode != 0:
            print(f"  ⚠️ Bailian 配额检查返回 {r.returncode} (console auth 过期或 quota 不可查, ASR 仍可用, Groq only 不启用 bailian)")
            # 2026-07-30: 不 fatal, 主路 groq 正常即可跑批
    except Exception as e:
        print(f"  ❌ Bailian 配额检查失败: {e}")
        fatal = True
    
    # 2. Groq key
    print("\n[2/4] Groq API key")
    try:
        from common.transcribe import check_groq_balance, get_groq_key
        key = get_groq_key()
        if key and check_groq_balance(key):
            print("  ✅ Groq key 可用")
        else:
            print("  ❌ Groq key 不可用（Groq only, 无 fallback, 本批转录将全部失败）")
    except Exception as e:
        print(f"  ⚠️ Groq 检测失败: {e}")
    
    # 3. opencli — 2026-07-29 deprecated：全部走 API，不再 subprocess 检测
    print("\n[3/4] opencli (已弃用, 全部走 API, 不再检测)")
    print("  ⏭️  跳过 opencli profile list 检测（已弃用）")
    
    # 4. 凭证目录
    print("\n[4/4] 凭证目录")
    creds = Path.home() / ".agents" / "credentials"
    if creds.exists():
        n = len(list(creds.iterdir()))
        print(f"  ✅ 凭证目录存在（{n} 个文件）")
    else:
        print(f"  ❌ 凭证目录不存在: {creds}")
        fatal = True
...
def cmd_all(date: str, py: str, skill: str):
    """一气呵成: backfill_pending → watchlist → clip → 自动 report。

    流程 (2026-08-03 修 #19 集成自动化):
    - Step 0: preflight (模型/工具检测)
    - Step 0.5: backfill_pending (自动消化上次遗留 wav, 让 commit 8d5252f 持久化真正生效)
    - Step 1: watchlist (走 supervisor 内嵌)
    - Step 2: clip (走 supervisor 内嵌, reset=False 保留 watchlist 阶段 timing)
    - Step 3: auto report (独立 subprocess 调 generate_op_report.py, 跑批失败也能补报告)
    """
    import subprocess
    from pathlib import Path
    skill_path = Path(skill)
    crawl_py = skill_path / "common-flow" / "crawl.py"
    backfill_py = skill_path / "tools" / "backfill_pending.py"
    report_py = skill_path / "tools" / "generate_op_report.py"

    run_tag = run_meta.make_run_tag()
    run_meta.append_event(run_tag, "run_started", date=date)
    print("=" * 60)
    print(f"📦 ominicrawl all 流程 - 日期: {date} run_tag={run_tag}")
    print("=" * 60)

    # Step 0: preflight (模型/工具检测)
    preflight_ok = preflight_check(skill, py)
    if not preflight_ok:
        print("\u274c preflight 有致命问题 (e.g. 凭证目录缺失), 拒绝跑批")
        print("   修复后重新触发 '爬取'")
        return 1

    # Step 0.5: backfill_pending (跨 batch retry, 让 commit 8d5252f 持久化真生效)
    # 独立 subprocess 调, 不走 supervisor (backfill 不需要看门狗, 失败不应阻断 watchlist)
    print("\n[0.5/3] 🔄 backfill pending wav (跨 batch retry)\n")
    if backfill_py.exists():
        rc_bf = subprocess.run([py, str(backfill_py)]).returncode
        if rc_bf not in (0, 2):  # 2 = 部分失败但已跑, 仍继续
            print(f"⚠️ backfill_pending 退出码 {rc_bf}, 继续 watchlist")
    else:
        print(f"⚠️ {backfill_py} 不存在, 跳过 backfill")

    # Step 1: watchlist (走 supervisor 内嵌)
    print("\n[1/3] 🚀 watchlist 阶段\n")
    rc1 = run_supervised([py, str(crawl_py), "watchlist", "--date", date], run_tag=run_tag)
    if rc1 != 0:
        print(f"⚠️ watchlist 退出码 {rc1}，继续 clip")

    # Step 2: clip (走 supervisor 内嵌, reset=False 保留 watchlist 阶段 timing)
    print("\n[2/3] 📋 clip 阶段\n")
    rc2 = run_supervised([py, str(crawl_py), "clip"], reset=False, run_tag=run_tag)
    if rc2 != 0:
        print(f"⚠️ clip 退出码 {rc2}，继续 report")

    # 先封口事件流，再生成报告；否则报告会看不到 run_finished，拿不到完整墙钟。
    run_meta.append_event(run_tag, "run_finished", exit_code=0 if rc1 == 0 and rc2 == 0 else 1)

    # Step 3: 自动生成 OP 报告 (独立 subprocess, 跑批失败也能补报告)
    # 2026-08-03 修 #19: 不再提示手动执行, 直接调 tools/generate_op_report.py
    print("\n[3/3] 📊 自动生成 OP 报告\n")
    if report_py.exists():
        rc_rep = subprocess.run([py, str(report_py), "--date", date, "--run-tag", run_tag]).returncode
        if rc_rep != 0:
            print(f"⚠️ generate_op_report 退出码 {rc_rep}")
    else:
        print(f"⚠️ {report_py} 不存在, 跳过 report")

    # 2026-08-14 fix #index: VM daemon 在队列清空时自动重建最近 2 天 index，
    # 治本「supervisor 跑批结束立即重建 → VM daemon 还在异步处理 → index 漏列」。
    # Mac 端不再重复重建（避免用未同步的 vault 状态写覆盖）。
    print("\n[4/4] ⏭️  跳过 Mac 端 index 重建（VM daemon 队列清空时已重建）\n")
    try:
        # 保留 import 以便手工触发：python -m common_supervisor._gen_index --date YYYY-MM-DD
        from common_supervisor._gen_index import regenerate_recent  # noqa: F401
    except Exception as e:
        print(f"  ⚠️ _gen_index 模块导入失败（不影响主流程）: {e}")

    # ✅ 新增: 爬取结束自动退出 Chrome (清理 worker tabs + 独立 Chrome 实例)
    from common.opencli_bridge import cleanup_tabs, cleanup_isolated_chrome_window
    cleanup_tabs()
    cleanup_isolated_chrome_window()
    print("  ✅ Chrome 清理完成 (worker tabs + 独立实例已关闭)")

    return 0


# _build_op_markdown 已迁移到 tools/generate_op_report.py (2026-08-01: 修 #17 — supervisor 只负责跑批 + 看门狗, 不再同步生成报告)

def main():
    parser = argparse.ArgumentParser(prog="supervisor.py",
                                     description="ominicrawl 主动监护主程序")
    parser.add_argument("--sub", nargs=argparse.REMAINDER, dest="sub_cmd",
                        help="子进程命令（supervisor.py --sub python crawl.py watchlist）")
    parser.add_argument("--status", action="store_true",
                        help="只读状态文件")
    parser.add_argument("--reset", action="store_true",
                        help="重置所有状态文件（恢复初始）")
    parser.add_argument("--all", nargs="?", const="today", default=None,
                        help="一气呵成：watchlist → clip → report（唯一推荐入口）")
